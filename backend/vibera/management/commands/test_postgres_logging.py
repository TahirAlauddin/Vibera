...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not options.get("skip_docker_check"):
            ...
            self.stdout.write(
                self.style.WARNING("Skipping Docker check (--skip-docker-check)\n")
            )
    ...
    def _check_docker_environment(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"[OK] Docker is installed: {result.stdout.strip()}"
                    )
                )
            else:
                self.stdout.write(self.style.ERROR("[FAIL] Docker is not available"))
                return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.stdout.write(self.style.SUCCESS("[OK] Docker Desktop is running"))
            else:
                self.stdout.write(
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR("[FAIL] Cannot check container status"))
    ...
    def _start_postgres_container(self):
        ...
        if not compose_file.exists():
            self.stdout.write(
                self.style.ERROR(
                    f"[FAIL] docker-compose.yml not found at {compose_file}"
                )
            )
            return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if not self.test_results["container_running"]:
                    self.stdout.write(
                        self.style.ERROR(
                            "[FAIL] PostgreSQL container did not become ready in time"
                        )
                    )
            else:
                self.stdout.write(
                    self.style.ERROR(
                        f"[FAIL] Failed to start container: {result.stderr}"
                    )
                )
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                if result:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            "[OK] Database connection created and logged"
                        )
                    )
                    self.stdout.write('  Check logs for: "Database connection created"')
            ...
            self.stdout.write('  Check logs for: "Database connection closed"')
    ...
    def _test_query_logging(self):
        ...
        if not self._is_postgresql():
            self.stdout.write(
                self.style.WARNING(
                    "[WARN] Skipping query logging test (not PostgreSQL)"
                )
            )
            return
        ...
        try:
            with connection.cursor() as cursor:
                ...
                if result:
                    self.stdout.write(
                        self.style.SUCCESS(
                            "[OK] INSERT and UPDATE queries executed and logged"
                        )
                    )
    ...
    def _test_slow_query_detection(self):
        ...
        try:
            ...
            threshold_ms = float(
                getattr(settings, "DB_SLOW_QUERY_THRESHOLD_MS", 1000.0)
            )
            sleep_seconds = (
                threshold_ms / 1000.0
            ) + 0.5  # Sleep slightly longer than threshold

            self.stdout.write(f"  Slow query threshold: {threshold_ms}ms")
            self.stdout.write(f"  Executing query with {sleep_seconds:.1f}s delay...")
            ...
            self.stdout.write(self.style.SUCCESS("[OK] Slow query executed and logged"))
            self.stdout.write('  Check logs for: "Slow query detected" warning')
    ...
    def _test_error_logging(self):
        ...
        if not self._is_postgresql():
            self.stdout.write(
                self.style.WARNING(
                    "[WARN] Skipping error logging test (not PostgreSQL)"
                )
            )
            return
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS("[OK] Invalid SQL error logged"))
            self.stdout.write(
            ...
            self.stdout.write(
                self.style.ERROR(
                    f"[FAIL] Error logging test (invalid SQL) failed: {str(e)}"
                )
            )
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(
                    f"[FAIL] Error logging test (constraint) failed: {str(e)}"
                )
            )
    ...
    def _verify_log_output(self):
        ...
        if not self.log_file_path or not self.log_file_path.exists():
            self.stdout.write(
                self.style.WARNING(
                    "[WARN] Log file not found, skipping file verification"
                )
            )
    ...
    def _check_docker_logs(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"[FAIL] Failed to retrieve Docker logs: {result.stderr}"
                    )
                )
    ...
    def _print_summary(self):
        ...
        if self.test_results["docker_available"]:
            status = (
                "[OK] Available"
                if self.test_results["docker_running"]
                else "[FAIL] Not running"
            )
