...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        synced_users = list(users)
        if connect_username:
            main_user = User.objects.filter(username=connect_username).first()
            if main_user and main_user not in synced_users:
                synced_users.append(main_user)
